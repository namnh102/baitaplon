...
def main():
    ...
    parser.add_argument('--Attribute', type=str, required=True,
                        help='List of attributes to compare, separated by commas')

    args = parser.parse_args()

    # Đọc dữ liệu từ file CSV
    data = pd.read_csv(r'BTLPYTHON\results.csv')

    player1 = args.p1
    player2 = args.p2
    attributes = args.Attribute.split(',')

    # Chuyển đổi các giá trị thuộc tính về kiểu số
...
# Ví dụ cách chạy mã
# python BTLPYTHON\Phan3-2.py --p1 "Aaron Cresswell" --p2 "Aaron Wan-Bissaka" --Attribute "Age,Matches Played,Starts,Minutes,Non-Penalty Goals,Penalties Made,Assists,Yellow Cards,Red Cards"